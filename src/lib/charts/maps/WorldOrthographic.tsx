...
// The orthographic globe always uses the full-world topology — regional
// topology presets (used by the Robinson `World` map) are not applicable.
const loadTopology = createTopologyLoader(() => import('../../data/maps/world/countries-50m.json'));
...
const WorldOrthographic = () => {
	...
	// the starting view and re-seeds whenever the editor view controls change.
	...
	const handlePointerDown = useCallback(
		...
		[dragToRotate, wpEditorFunctions]
	);
						...
						{({ features }) => {
							...
							const labelLayer = labels.active
								? featureMeta.map((meta) => {
										...
										const customLabel = customLabelText || getCustomLabel(properties, mapCategory);
